import TransitionManager from '@deck.gl/core/dist/es6/controllers/transition-manager';
...
export default class Controller {
    ...
    keyboard: any;
    invertPan: boolean;
    private _panMove: any;
    inertia: any;
    private _startPinchRotation: any;
    private _lastPinchEvent: any;
    ...
    _onPinchStart(event) {
        const pos = this.getCenter(event);
        console.log('_onPinchStart', pos, event.rotation, event.scale);
        if (!this.isPointInBounds(pos, event)) {
            return false;
        }
